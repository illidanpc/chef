...
bash 'run_rdbms_installer' do
    ...
    code "sudo -Eu oracle ./runInstaller -showProgress -silent -waitforcompletion -ignoreSysPrereqs -responseFile #{node[:rac][:rdbms][:install_dir]}/db11R23.rsp -invPtrLoc #{node[:rac][:ora_base]}/oraInst.loc"
    returns [0, 6]
  end
